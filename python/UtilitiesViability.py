...
def ReprodBiAllelic(freq):
    p = freq[0,:]/np.sum(freq,0)
    q = freq[1,:]/np.sum(freq,0)
    pPrime = p**2 + np.multiply(p,q)
    qPrime = q ** 2 + np.multiply(p,q)
    freqPrime = np.vstack((pPrime,qPrime))*np.sum(freq,0)
    
    return freqPrime
    
def ReproTriAllelic(freq):
    p1 = freq[0,:]/np.sum(freq,0)
    p2 = freq[1,:]/np.sum(freq,0)
    p3 = freq[2,:]/np.sum(freq,0)
